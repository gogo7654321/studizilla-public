...
import { updateCardReportStatus } from './actions';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
type ReportStatus = 'pending' | 'approved' | 'denied';

type CardReport = {
  id: string;
  reporter: { uid: string; name: string };
  deck: { id: string; title: string; ownerId: string };
  card: { id: string | number; term: string; definition: string };
  reason: 'incorrect' | 'inappropriate' | 'typo' | 'other';
  details?: string;
  status: ReportStatus;
  createdAt: Timestamp;
};

const statusVariant: Record<ReportStatus, 'default' | 'secondary' | 'destructive'> = {
    pending: 'default',
    approved: 'secondary',
    denied: 'destructive',
};
...
export function CardModeration() {
    const [reports, setReports] = useState<CardReport[]>([]);
    const [isLoading, setIsLoading] = useState(true);
    const { toast } = useToast();

    useEffect(() => {
        const q = query(collection(db, 'cardReports'), orderBy('createdAt', 'desc'));
        const unsubscribe = onSnapshot(q, (querySnapshot) => {
            const fetchedReports: CardReport[] = [];
            querySnapshot.forEach((doc) => {
                fetchedReports.push({ id: doc.id, ...doc.data() } as CardReport);
            });
            ...
            toast({
                variant: 'destructive',
                title: 'Error',
                description: 'Could not load card moderation queue.'
            });
        });
    ...
    const handleAction = async (id: string, newStatus: 'approved' | 'denied') => {
        try {
            await updateCardReportStatus(id, newStatus);
            toast({
                title: `Report ${newStatus}`,
                description: `The report has been updated successfully.`
            });
        } catch (error: any) {
            console.error("Error updating card report:", error);
             toast({
                variant: 'destructive',
                title: 'Action Failed',
    ...
    return (
        <Card className="rounded-2xl">
            <CardHeader>
                <CardTitle>Flashcard Report Queue</CardTitle>
                <CardDescription>Review and manage user-submitted reports on individual flashcards.</CardDescription>
            </CardHeader>
            <CardContent>
                <ScrollArea className="h-[600px] border rounded-xl">
                    {isLoading ? (
                        ...
                        <div className="flex items-center justify-center h-full">
                            <p className="text-muted-foreground">No card reports found. Great job!</p>
                        </div>
                    ) : (
                        <Table>
                            <TableHeader>
                                <TableRow>
                                    ...
                                    <TableHead>Reported Card</TableHead>
                            ...
                            <TableBody>
                                {reports.map(req => (
                                    <TableRow key={req.id}>
                                        ...
                                        <TableCell>
                                            <p className="font-semibold text-sm truncate max-w-xs">{req.card.term}</p>
                                            <p className="text-xs text-muted-foreground truncate max-w-xs">{req.card.definition}</p>
                                        </TableCell>
                                             ...
                                             <AlertDialog>
                                                ...
                                                <AlertDialogContent>
                                                    ...
                                                    <div className="bg-secondary rounded-xl p-4 my-4 space-y-2">
                                                      ...
                                                      <p className="text-sm"><strong>Deck Owner:</strong> {req.deck.ownerId}</p>
